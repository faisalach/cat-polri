...
<script>
	$(function(){
		...
		$("body").on("change",".choice_type",function(e) {
			e.preventDefault();
			var choice_type = $(this).val();
			if (choice_type == 'hybrid') {
				$(this).closest('.card').find('.correct_choice').attr('type','checkbox');
			}else{
				$(this).closest('.card').find('.correct_choice').attr('type','radio');
			}
		})
		$("body").on("change",".correct_choice",function(e) {
			var card 		= $(this).closest('.card');
			var choice_type = card.find('.choice_type').val();
			var isChecked 	= $(this).is(":checked");
			if (isChecked && choice_type == 'hybrid') {
				if (card.find(".correct_choice:checked").length > 2) {
		...
		function htmlQuestion(count) {
			...
			for(let i = 0; i < count;i++){
				...
				@if($course->course_category_id == 1)
				<div class="form-group">
				<p class="mb-2">Mode Opsi</p>
				<select name="choice_type[${i}]" class="form-control choice_type">
				<option value="normal" ${data[i] != undefined && data[i].choice_type == 'normal' ? 'selected' : ''  }>Normal</option>
				<option value="hybrid" ${data[i] != undefined && data[i].choice_type == 'hybrid' ? 'selected' : ''  }>Hybrid</option>
				</select>
				</div>
				@endif
				<div class="form-group">
				...
				<input name="correct_choices[${i}][]" ${data[i] != undefined && (data[i].correct_choice == '{{ $val }}' || data[i].correct_choice_2 == '{{ $val }}') ? 'checked' : ''  } value="{{ $val }}" type="${data[i] != undefined && data[i].choice_type == 'hybrid' ? 'checkbox' : 'radio'  }" class="mr-3 correct_choice">
				<strong class="mr-3">{{ $val }}.</strong>
				<div class="w-100">
				<input class="form-control d-none" type="text" name="choices[${i}][{{$val}}]" value="${data[i] != undefined && data[i].choices['{{ $val }}'] != undefined ? data[i].choices['{{ $val }}'] : ''  }" placeholder="Masukkan Jawaban Opsi {{$val}}">
				</div>
				</div>
				@endforeach
				</div>
